...
proxies:
  - {name: "${ClashMetaName}", type: hysteria, server: ${IP}, port: ${PORT}, obfs: ${OBFS}, protocol: udp, up: 2000, down: 2000, skip-cert-verify: true}
proxy-groups:
  - name: 🚀 节点选择
    type: select
    proxies:
...
Restart=always
TEXT
    url="hysteria://${IP}:${PORT}?auth=${OBFS}&upmbps=2000&downmbps=2000&obfs=xplus&obfsParam=${OBFS}"
